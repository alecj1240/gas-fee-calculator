...
function App() {
  ...
  getTxs('0xaC97AFb90dFDcB534F6D242B911DcA37cC4382b6').then((txs) => {
    console.log(txs);
  });
...
async function getTxs(address) {
  ...
  chainConfig['0xfa'] = {id: '0xfa', shortname: 'ftm', name:'Fantom', symbol: 'ftm', coingecko_name: 'fantom', token: 'ƒ', color: '#00dbff', explorer_uri: 'https://api.ftmscan.com', key: 'B5UU3GDR3VJYVXFYT6RPK5RA6I8J5CV6B3'}
    ...
    if (nOut > 0) {
        ...
        gasPriceTotal = comma((gasPriceTotal / nOut / 1e9).toFixed(1));
        gasFeeTotal = chainConfig[chainId].token + comma((gasFeeTotal / 1e18).toFixed(3));
        
        if (nOutFail > 0) {
            gasFeeTotalFail = chainConfig[chainId].token + (gasFeeTotalFail / 1e18).toFixed(3);
      ...
      gasUsedTotalState = 0;
      gasFeeTotal = chainConfig[chainId].token + 0;
    }

    return [gasUsedTotal, gasPriceTotal, gasFeeTotal, gasFeeTotalFail];
  }
